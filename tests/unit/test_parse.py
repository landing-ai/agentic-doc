...
class TestFigureCaptioningAndChunking:
    ...
    def test_parse_with_figure_captioning_config_override(self, temp_dir, mock_parsed_document):
        """Test that ParseConfig overrides figure captioning parameters."""
        test_file = temp_dir / "test.pdf"
        with open(test_file, "wb") as f:
            f.write(b"%PDF-1.7\n")
        ...
        with patch("agentic_doc.parse._parse_pdf", return_value=mock_parsed_document):
            result = parse(
                test_file,
                figure_captioning_type=FigureCaptioningType.verbose,  # Should be overridden
                split=SplitType.full,  # Should be overridden
                config=config
            )
